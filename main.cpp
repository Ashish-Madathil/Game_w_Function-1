...
{  
  ...
  getUserNames(playerNames); //pass by reference is a default for arrays
  ...
  do
  {
    ...
    while (gameOver == false)
      ...
      player1Turn = !player1Turn;
	    ...
	    {
	 	    gameOver = true;
	      cout << FindPlayerName(playerNames, player1Turn) << ", congratulations you won\n"; // parameter of FindPlayerName: inside main() it's playerNames, outside
      }                                                                                    // it's names because playerNames is declared inside main().
	    
	  }
    cout << "Do you wish to play again? (Y/N)\n";
    cin >> userChoice;
    userChoice = toupper(userChoice); // convert userChoice to the uppercase version of itself, using toupper from cstdlib
  }while (userChoice == 'Y');  
  return 0; 
}
